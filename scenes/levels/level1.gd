...
func start_conversation(button: Button, dialogue: DialogueResource, start_line_id: String):
	if button.visible and conversation_agent.start_conversation_if_idle(dialogue, start_line_id):
		button.visible = false
		time.potential_talkmate_count -= 1

func think(line_id: String):
	var button := %Me/Button
	button.pressed.connect(func(): start_conversation(button, dialogue, line_id))
